...
function Map:init()
    ...
    while x < self.mapWidth do
        ...
        -- 8.33% chance to not generate anything, creating a gap
        ...
        -- generate empty tiles so that nothing renders in front of flag
        -- doesn't work because TILE_EMPTY is not a TILE as such
        for y = self.mapHeight / 2, self.mapHeight / 8 do
            for x = self.mapWidth / 2 + 4, self.mapWidth do
                self:setTile(x, y, TILE_EMPTY)
                x = x + 1
                y = y + 1
            end
        ...
        while x == self.mapWidth / 2 + 4 do
            ...
            -- generates TILE_BRICK soon as pyramid starts
            for y = self.mapHeight / 2, self.mapHeight do
                for x = self.mapWidth / 2 + 4, self.mapWidth do
                    self:setTile(x, y, TILE_BRICK)
                end
...
function Map:render()
    ...
    -- displaying victory message if we hit the flag
    if gameState == 'Victory' then
        love.graphics.setDefaultFilter('nearest', 'nearest')
        love.graphics.setFont(love.graphics.newFont('fonts/font.ttf', 15))
        love.graphics.printf('Level Completed!',0, VIRTUAL_HEIGHT / 6,  VIRTUAL_WIDTH, 'right')
    end
end
