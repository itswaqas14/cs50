...
function Player:update(dt)
    ...
    if self.dy < 0 then
        if self.map:tileAt(self.x, self.y).id == JUMP_BLOCK or
            ...
            self.map:tileAt(self.x + self.width - 1, self.y).id == JUMP_BLOCK then
            -- reset y velocity
            self.dy = 0
        ...
        if self.map:tileAt(self.x, self.y).id == POLE_MID or
            self.map:tileAt(self.x, self.y - 1).id == FLAG or
            self.map:tileAt(self.x, self.y - 1).id == POLE_BOTTOM then
                gameState = 'Victory'
        end
    end
